...
void link::userData(int fileNum){
	int n;
	for(int i = 0; i < fileNum; i++){
		...
		if(n <= max_value - 1){
			...
			cout << "\nEntered item exceeds total IDSpace value... enter again!\n";
			i--;
		}
	}
}
...
bool link::errorCheck(int num){
	int val = errorCheck(head, num); 
	return val;
}

bool link::errorCheck(node * head, int num){
	node* current = head;
    while(current != NULL){
    	if(num == current->data){
    		return 1;
...
void link::lookup(int key, int number){
	...
	if(k == 0){
		cout << "\nEntered key is not in the list... cannot look up key...\n";
		return;
	}
}
