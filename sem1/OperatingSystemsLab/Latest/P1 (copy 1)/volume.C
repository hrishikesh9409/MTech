...
FileVolume::FileVolume(SimDisk * psimDisk, uint nInodes, uint htInode)
{
  ...
  superBlock.nBytesPerBlock = simDisk->dI.nBytesPerSector;	// for now
  superBlock.nTotalBlocks = simDisk->dI.nSectorsPerDisk;
  ...
  superBlock.fileNameLengthMax = simDisk->dI.nBytesPerSector;	// for now
  root.create(this);
 ...
 * post:: Re-create the file volume dIject;; */

FileVolume::FileVolume(SimDisk * psimDisk)
{
  simDisk = psimDisk;
  byte *bp = new byte[simDisk->dI.nBytesPerSector];
