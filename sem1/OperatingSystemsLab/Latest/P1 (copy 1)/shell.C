...
#include "fstypes.h"
unsigned int maxF,n_Inodes,inodeh; //declare variables for maxFnm n_Inodesodes iNodeHt
...
{
  char name[1024] = "";
  uint nblocks = 0, nbytes = 0,maxFnm,n_Inodesodes,iNodeHt;
  ...
  while(!feof(f)){
   if(strcmp(s,name)==0)
	break;
   fscanf(f, "%s %u %u %u %u %u\n", name, &nblocks, &nbytes, &maxFnm, &n_Inodesodes, &iNodeHt);
   maxF=maxFnm;			//assign_Inodesg values
