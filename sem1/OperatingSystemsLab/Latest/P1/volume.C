...
FileVolume::FileVolume(SimDisk * psimDisk, uint nInodes, uint htInode)
{
  ...
  superBlock.nBytesPerBlock = simDisk->ob.nBytesPerSector;	// for now
  superBlock.nTotalBlocks = simDisk->ob.nSectorsPerDisk;
  ...
  superBlock.fileNameLengthMax = simDisk->ob.nBytesPerSector;	// for now
  root.create(this);
...
{
  simDisk = psimDisk;
  byte *bp = new byte[simDisk->ob.nBytesPerSector];
