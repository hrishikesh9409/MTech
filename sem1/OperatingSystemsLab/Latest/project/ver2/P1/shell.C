...
unsigned int mf,nin,inh, no_blocks, Bytes; //declare variables for maxFnm nInodes iNodeHt
...
void doMakeDisk(Arg * a)
{
  ...
  str=a[0].s;
  uint r = 0;
  if(strcmp(str, "D1") == 0 || strcmp(str, "D2") == 0 || strcmp(str, "D3") == 0 || strcmp(str, "D4") == 0 || strcmp(str, "D5") == 0)
  	r = mkSimDisk(str);
  else{
  	printf("\nCannot create Disk %s\n", str);
  	printf("Enter Disk between 1 and 5 only\n");
  	return;
  }
  if (r != 0)
    releaseCfv();
...
void doWriteDisk(Arg * a)
{
  ...
  if(simDisk == 0){		
    printf("Cmd wrdisk needs the simDisk to be != 0\n");		
    return ; //return to main (goes to invokeCmd(..) but it the last line)		
  }
...
void doReadDisk(Arg * a)
{
  char buf[1024];		// assuming nBytesPerSectorMAX < 1024
  if(simDisk == 0){		
    printf("Cmd rddisk needs the simDisk to be != 0 \n");		
    return ; //return to main (goes to invokeCmd(..) but it is the last line)		
  }
...
void doMakeFS(Arg * a)
{
  //checking wheather the disk is the same or not.
  if(strcmp(a[0].s,simDisk->ob.labelOfTheDisk)==0){
	char name[1024] = "";
  uint nblocks = 0, nbytes = 0,maxFnm,nInodes,iNodeHt;
 ...
 // uint ht = a[1].u;
  ...
  releaseCfv();
  if(simDisk == 0){		
    printf("Cmd mkfs needs the simDisk to be != 0 \n");		
    return ; //return to main (goes to invokeCmd(..) but it is the last line)		
  }
...
void doFindFS(Arg * a)
{
  //findfs checks if disk exists
  if(simDisk!=0){
  //findfs checks if volume exists
    if(cfv!=0){
  releaseCfv();
  if(simDisk == 0){		
    printf("Cmd findfs needs the simDisk to be != 0 \n");		
    return ; //return to main (goes to invokeCmd(..) but it is the last line)		
  }  
  cfv = simDisk->findfs();
  printf("findfs() == %px\n", (void *) cfv);
  ...
  printf("filesystem doesnot exists\n");}
else
...
void doCreateFile(Arg * a){
  char * c = a[0].s;
  if(strlen(c) <= mf){      //Check for size of file name
    if(isAlphaNumDot(c[0]) == 1 && isDigit(c[0]) != 1){ //Check for alpha numeric and numeric characters 
      cfv->root.createFile(c, cfv);
    }
    else
...
void doList(Arg * a){
  cfv->root.ls(cfv, no_blocks, Bytes, mf);
}
...
void invokeCmd(int k)
{
  ...
  if (cmdTable[k].globalsNeeded[0] == 'd' && simDisk == 0) {
    //to directly call the already created Disk

      if (strcmp("fddisk", cmdTable[k].cmdName) != 0){ //added line
        ok = 0;
