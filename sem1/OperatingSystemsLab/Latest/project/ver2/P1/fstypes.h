...
extern unsigned int mf,nin,inh, no_blocks, Bytes;   // Declaring globally the variables for maxFnm nInodes iNodeHt
...
public:
  ...
  uint createFile(char *nm, FileVolume * pfv);
  uint ls(FileVolume * pfv, uint no_blocks, uint Bytes, uint mf);
  uint rm(char *nm, FileVolume * pfv);
private:
