...
SimDisk::SimDisk(const char *name, uint iSectorsPerDisk, uint iBytesPerSector)
{
  if (name == 0 || *name == 0)
    name = "D0";
  strncpy(dI.labelOfTheDisk, name, LabelSZ);
  dI.nSectorsPerDisk = iSectorsPerDisk;
 dI.nBytesPerSector = iBytesPerSector;
  if (paramsOK() == 0)
    return;
  ...
  if (fd > 0) {
    ...
    fstat(fd, &statBuf);
    close(fd);
    exists = (uint) statBuf.st_size == dI.nSectorsPerDisk *dI.nBytesPerSector;
  }
  if (exists == 0)
    makeDiskImage();
}
...
{
  return
      0 < dI.nSectorsPerDisk && dI.nSectorsPerDisk <= SectorsMAX &&
      0 < dI.nBytesPerSector && dI.nBytesPerSector <= BytesPerSectorMAX;
}
...
{
  char tfnm[1024];
  sprintf(tfnm, "%s.dsk", dI.labelOfTheDisk);
...
int SimDisk::makeDiskImage()
{
  ...
  char *buf = new char[dI.nBytesPerSector];
  memset(buf, '3',dI.nBytesPerSector);
  for (uint i = dI.nSectorsPerDisk; i--;){
    if(write(fd, buf, dI.nBytesPerSector) != dI.nBytesPerSector){
...
{
  if (p == 0 || nBlock >= dI.nSectorsPerDisk)
    return 0;
  int fd = openDiskImage(O_RDONLY);
  lseek(fd, dI.nBytesPerSector * nBlock, SEEK_SET);
  int readCount = read(fd, p, dI.nBytesPerSector);
  if(readCount == -1){    
      printf("Read failed in SimDisk::readSector \n");    
      exit(0);    
  }
  close(fd);
  return dI.nBytesPerSector;
}
...
uint SimDisk::writeSector(uint nBlock, void *p)
{
  ...
  lseek(fd, dI.nBytesPerSector * nBlock, SEEK_SET);
  unsigned int charCount = write(fd, p, dI.nBytesPerSector);   
  if(charCount != dI.nBytesPerSector){   
