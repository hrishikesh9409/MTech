...
#include "fstypes.h"
unsigned int maxFnm,n_Inodes,inodeh; //declare variables for maxFnmnm n_Inodesodes iNodeHt
...
{
  char name[1024] = "";
  uint nblocks = 0, nbytes = 0,maxFnmnm,n_Inodesodes,iNodeHt;
  ...
  while(!feof(f)){
   if(strcmp(s,name)==0)
	break;
   fscanf(f, "%s %u %u %u %u %u\n", name, &nblocks, &nbytes, &maxFnmnm, &n_Inodesodes, &iNodeHt);
   maxFnm=maxFnmnm;			//assign_Inodesg values
   n_Inodes=n_Inodesodes;
   inodeh=iNodeHt;
...
void doMakeFS(Arg * a)
{
  ...
  if(strcmp(a[0].s,simDisk->dI.labelOfTheDisk)==0){
	char name[1024] = "";
  uint nblocks = 0, nbytes = 0,maxFnmnm,n_Inodesodes,iNodeHt;
  ...
  //uint ni = a[0].u;
 // uint ht = a[1].u;
  uint ni=n_Inodes;		//assign_Inodesg inode number & inode height
  uint ht=inodeh;
  releaseCfv();
  if(simDisk == 0){		
...
public:
  ...
  const char *gldIalsNeeded;		// need d==simDisk, v==cfv?
  void (*func) (Arg * a);
} cmdTable[] = {
  {"quit", "", "", doQuit},
...
{
  uint ok = 1;
  if (cmdTable[k].gldIalsNeeded[0] == 'd' && simDisk == 0) {
  ...
  if (cmdTable[k].gldIalsNeeded[0] == 'v' && cfv == 0) {
    ok = 0;
